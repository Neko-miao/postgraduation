...
然而，正如前文所言，通过连续LoD技术来简化模型网格的研究已经相当成熟，但现存的相关技术并不能完全满足本文研究需求。例如，大多数通过连续LoD来简化模型网格的方案都是基于静态网格的，而本文需要研究如何简化动态网格（如角色动画蒙皮网格）。因此本文改进了基于边坍缩的网格简化算法，充分考虑骨骼运动对蒙皮网格顶点的影响，提出了一种GPU可并发的动画网格简化算法（Parallel Animation QSlim, PAQSlim），在提高群体动画渲染效率的同时通过为模型选择合适的LoD还能充分利用了GPU内存。  
### 4.2 并发网格简化算法改进  
...
#### 4.2.1 边坍缩代价估计改进
- 定义问题域  
    应用PAQSlim算分的关键一步是估计每条边的坍缩代价，考虑到动画蒙皮网格是随时间变化的，不能直接使用QME估计方法，因此本文综合了骨骼对动画蒙皮的影响，提出了一种适用于蒙皮网格的动态二次误差度量方法（Dynamic Quadric Error Metric, DQEM）。在DQEM估计中，一个动画网格被定义为$AnimMesh = (V, T, B)$。其中$V$是动画网格的顶点集合，$T$是动画网格的三角形集合，$B$是该动画对应骨架的所有骨骼集合。顶点集合中的每一个顶点$v \in V$都会受到至多4个骨骼的影响，用一个四元组$MeshWeight=(W_0, W_1, W_2, W_3)$表示，其中$W_i = (b_i, w_i)(i = 0, 1, 2, 3)$是一个二元组，$b_i$表示影响顶点位置的骨骼，$w_i$表示该骨骼的影响权重。骨骼集合中每一个骨骼$b \in B$表示其相对它父骨骼$p_b \in B \bigcup \emptyset$的空间变换，若$p_b = \emptyset$，则骨骼$b$就是骨架空间的根骨骼。一般情况下，骨骼$b$用一个$4 \times 3$矩阵表示它的位置、旋转和缩放信息，即$b = (M_b, pdf_b, p_b)$，$M_b = (T_b, R_b, S_b)$是一个表示骨骼$b$相对其父骨骼的局部空间变换矩阵，$T_b$是表示位置的三维向量，$R_b$是表示旋转的四元数，$S_b$是表示缩放的三维向量，最后$p_b$指向骨骼$b$的父骨骼，所有骨骼的局部变换矩阵组成了骨架的一个姿势。骨架的姿势并不是静态的，而是随着时间的增长而变换，本文将用一个概率密度分布函数来预测某一时刻的姿势状态。注意到在大多数的骨骼动画中，实际改变的数据往往是骨骼的旋转，因此定义$pdf_b$来描述骨骼$b$在其父骨骼空间的旋转概率分布情况，考虑到旋转的直观性和参数之间的相互独立性，本文没有直接在表示旋转的四元数空间上定义这个概率密度分布函数，而是在旋转对应欧拉角的三维空间上定义骨骼$b$旋转的联合概率分布函数，并合理假设三个维度的旋转相互独立，即有
    ...
    $Quaternion$是一个表示欧拉角转换到四元数的函数。同样的方法，可以得到骨骼$b$的父骨骼的旋转四元数，进而得到其局部空间变换矩阵矩阵$M_{p_b}$。对所有骨骼进行旋转采样，就可以确定所有骨骼的局部空间变换矩阵，进而得到骨架的一个随机姿势$P$，再对姿势$P$应用蒙皮，假设$v(P)$表示该蒙皮网格的一个顶点，则有
        $$v(P) = \sum_{i=0}^3w_i(M_{p_{b_i}}M_{b_i})M_{bind_{b_i}}^{-1}v(P_{bind}),where \sum_{i=0}^3w_i = 1$$
    其中，$b_i$和$w_i$分别表示影响该顶点的骨骼和对该顶点的影响权重，由该顶点的$MeshWeight$决定，$M_{bind_{b_i}}$表示骨骼$b_i$相对绑定姿势空间的变换矩阵，$v_(P_bind)$表示顶点$v$在绑定姿势下蒙皮位置。为了简化公式，设在姿势$P$下顶点$v$的蒙皮变换矩阵为$M_v(P) = \sum_{i=0}^3w_i(M_{p_{b_i}}M_{b_i})M_{bind_{b_i}}^{-1}$，而在绑定姿势$P_{bind}$下顶点$v$相对骨架空间的位置为$v_0 = v(P_{bind})$，于是上述公式可以简化为：
        $$v(P) = M_v(P)v_0$$

- QEM改进（基于姿势概率的积分）  
    基于以上工作，再结合第二章描述的QEM估计方法，在姿势$P$下每个蒙皮网格顶点$v$的二次误差度量矩阵$Q_v(P)$就可以计算得到。而一个动画中的骨架会有一系列姿势$P_i(i = 1, 2, 3, ...)$，所有姿势下顶点的二次误差度量的期望值就为该顶点最终的二次误差度量，即有
        $$ D(v) = \sum_{i=1}^{\infty}v(P_i)^TQ_v(P_i)v(P_i) \rho(P_i)  \\ 
            =  \sum_{i=1}^{\infty}(M_v(P_i)v_0)^TQ_v(P_i)(M_v(P_i)v_0) \rho(P_i)\\
            =  v_0^T(\sum_{i=1}^{\infty}M_v(P_i)^TQ_v(P_i)M_v(P_i)\rho(P_i))v_0 $$  
    其中$\rho(P_i)$表示采样得到姿势$P_i$的概率，且有$\sum_{i=1}^{\infty}\rho(P_i) = 1$。如果每根骨骼的旋转是相互独立的，那么一个姿势的概率密度分布就可以表示为：
        $$ pdf(P) = \prod_{b \in B}pdf_b = \prod_{b \in B}(pdf_b^{roll} pdf_b^{pitch} pdf_b^{yaw})$$
    本文提出的DQME估计方法正是用蒙皮顶点$v$的二次误差度量矩阵的期望值来估计顶点的二次误差度量，这个期望矩阵可以表示为
        $$Q_v = \sum_{i=1}^{\infty}M_v(P_i)^TQ_v(P_i)M_v(P_i) \rho(P)$$  
    考虑到$P$是一个可以用来表示骨架姿势的随机变量，且在其在空间上的变换是连续的，所以顶点$v$的二次误差度量期望矩阵可以该写成如下积分形式：
        $$Q_v = \int pdf(P)M_v(P)^TQ_v(P)M_v(P){\rm d}P$$  
- DQME实现（蒙特卡洛积分实现）  
    显然，直接对该积分进行求值是非常困难的，因为姿势$P$所在的空间维度是$3\|B\|$。受到蒙特卡洛积分的启发，将$pdf(P)$作为姿势$P$采样的概率密度分布函数，最终所有顶点的二次误差度量矩阵将由以下伪代码给出。
    ```
    Input: AnimMesh
    Output: Qv  
        for 1 to k
            P = GeneratePose()   // by pdf(P)
            SkinningMesh(P)
            for v in AnimMesh.V  
                Qv[v] += T(Mv(P))Qv(P)Mv(P)  // 
            end for
        end for
        for v in AnimMesh.V
            Qv[v] /= k
    ...
    ```
- 概率分布函数讨论  
    虽然上述讨论只涉及了骨骼旋转的变换，但是结合给出的公式可知，DQME方法完全可以拓展到骨骼的位移和缩放变换。而对于每个骨骼的概率密度分布函数，可以根据具体使用环境而合理设置。例如最简单的均匀分布，可以根据需要设置每个骨骼在三个自由度方向上旋转的最小值$min$和最大值$max$，并要求每个自由度上的旋转服从这一区间上的均匀分布，即概率密度分布函数为$1/(max - min)$。再如最常见的正太分布，这种分布更有利于表现骨骼运动的特性，其平均值$\mu$可以用来描述骨骼最有可能应用的旋转角度，而方差$\sigma$可以用来描述骨骼的刚性程度，一般来说对于动作幅度较大的角色，可以采用较大的方差值$\sigma$，反之亦然，而本文后续的实验部分正是采用正太分布来描述骨骼的旋转概率分布。总之，一个合理的概率密度分布函数应当契合角色动画特性来设置。
#### 4.2.2 边坍缩方向优化    
- 取消边界边合并的考虑  
    在第二章讨论的GPU并行网格简化算法中，原方法考虑了边界边对网格形状的影响并认为所有边界边的集合就是简化程度最高的网格，因此原方法忽略了边界边的坍缩，如果存在边界顶点和非边界顶点之间的坍缩，则严格限制只能由非边界顶点坍缩到边界顶点，否则根据顶点编号大小来决定坍缩方向。而在本文根据实验发现，在绝大多数情况下，边界顶点往往处于网格的锐利边缘，其计算得到的QME估计值往往是一个较大的值，这意味着和该边界顶点相关的所有边的坍缩代价都是一个偏大的值，因此正常执行QSlim算法也较难选择到一条带有边界顶点的边。所以，本文提出的PAQSlim算法没有考虑边界边以及边界顶点的影响，而是直接根据上一小节介绍的DQEM估计方法来选择坍缩代价最小的边进行边坍缩操作。事实上，QSlim算法实质就是保证所有顶点的二次误差度量是最小的，这也最大程度的保持了网格原有的外观形状，所以取消对边界顶点坍缩操作的考虑是合理的。
    在不考虑边界顶点对坍缩操作影响的前提下，原论文方法处理边坍缩方向时，始终将编号较小的边坍缩到编号较大的边，这么做的目的是方便后续对网格的顶点和三角形编号进行重排列以保证后续三角形网格重构的顺利。但仔细研究原论文发现，$ECol$数组存储边坍缩的操作，$PermuteV$数组也暗示了边坍缩的方向，因此在一条边的坍缩操作中，不需要限定编号较小的顶点一定要坍缩到编号较大的顶点，也能正确地执行网格重排列。所以，本文优化了原论文对边坍缩方向处理，在实际坍缩过程中，确保坍缩后目标顶点的DQME估计是最小的。优化后，边坍缩操作方程描述如下：
        $$Collapse(v_1, v_2) = \begin{cases}  
        v_1 \rightarrow v_2, & D(v_1) > D(v_2) \\
        v_2 \rightarrow v_1, & D(v_1) < D(v_2)  \\
        \end{cases}$$
    其中，$v_1$和$v_2$分别表示待坍缩边的两个顶点，D(v_1)和D(v_2)表示边坍缩到顶点$v_1$或顶点$v_2$后，坍缩的目标顶点对应的DQEM估计值，其计算方法是
        $$D(v_i) = v_i^T(Q_{v_1} + Q_{v_2})v_i, \space where \space \space i = 1, 2$$  
    这里的$Q_{v_1}$或者$Q_{v_2}$均由上一小节介绍的DQEM方法计算所得。
#### 4.2.3 LoD选择改进   
- 取消边界，总顶点数对比N，补足最简网格的顶点  
    原并行网格简化算法选择LoD如下：
    $$vc_i = N \frac{w_i^{1/\alpha}}{\sum_{i = 1}^{m}w_i^{1/\alpha}}, \space where \space w_i = \beta \frac{A_i}{D_i}P_i^{\beta},\beta = \alpha - 1$$
    各符号的含义已在第二章解释，这里不再赘述。需要指出的是，以上公式只是对网格顶点数量的初步选择，原论文还综合考虑了网格顶点数量的合理取值范围以及GPU的内存预算，并剔除了一些顶点数量足够小的网格，这种做法不仅充分考虑了GPU的内存分配，而且能够为场景的每一个网格选择出合适的分别率。而在上一小节，本文取消了边界顶点坍缩的限制，因此需要对LoD选择方法进行相对应的改进。假设$M$表示原网格的顶点数量，$\alpha \in (0, 1)$为最低分辨率因子，$vc_i$表示上述公式计算所得的某个网格的顶点数量，那么改进后，该网格分辨率将由以下公式给出：
           $$\bar{vc_i} = \begin{cases}  
            vc_i  & (vc_i \in [\alpha M, M]) \\
            max_i & (vc_i > M) \\
            N/A & (vc_i < \alpha M)\\
            \end{cases}$$  
    其中，$\bar{vc_i}$就表示网格最终的会被渲染的顶点数量，而$N/A$表示暂时不能确定，本小节将稍后给出对于这一部分网格顶点的处理方法。
    仔细观察以上公式不难发现，不考虑$N/A$部分的话，所有网格顶点数量之和在经过LoD选择后可能会小于给GPU分配的顶点预算$N$，剩余的预算部分将填充的$N/A$对应的网格中。假设$N'$表示已经确定顶点数量的网格顶点的总和，$m$表示还未确定顶点数量的网格（即$N/A$部分对应的网格）个数，那么可以填充到$N/A$对应的网格数量数量就是$r = (N - N')/(\alpha M)$，所以对于$m$个还未确定顶点数量的网格，从中任选选取$r$将其网格顶点填充为$\alpha M$。如果$m <= r$，那么所有网格都不会被抛弃，它们将最终被渲染在屏幕上，如果$m > r$，那么将有$m-r$个网格被剔除，不会进入后续渲染流程。值得说明的是，根据公式可知，在合理决策GPU顶点总预算$N$的前提下，如果存在被剔除的网格，那么它们距离相机的位置相对较远，即是被渲染在屏幕上也很难被用户发现，因此，剔除这部分网格并不会对最终渲染图像的视觉质量造成损害，本文实验部分的内容也证明了这点。  
