...
第二章介绍了一种可在GPU并行的网格简化算法，可以在运行时快速得到由LoD选择出的低分辨率网格，非常适合渲染大批量的网格。但是这种网格简化算法适用于静态网格，而本文研究内容是大规模群体动画的实时渲染，研究对象是动态的蒙皮网格。因此，本文改进了GPU并行的网格简化算法，综合考虑角色骨架变化对动画蒙皮网格的影响，提出了一种可支持GPU并行的动画网格简化算法——PAQSlim算法。本小节将详细介绍本文PAQSlim算法。
#### 4.2.1 动态二次误差估计方法（DQME算法）    
- 定义问题域  
本文研究内容之一是动画网格的简化，因此定义一个动画网格为$AnimMesh = (V, T, B)$。其中$V$是动画网格的顶点集合，$T$是动画网格的三角形集合，$B$是该动画对应骨架的所有骨骼集合。顶点集合中的每一个顶点$v \in V$都会受到至多4个骨骼的影响，用一个四元组$MeshWeight=(w_0, w_1, w_2, w_3)$表示，$w_i(i = 0, 1, 2, 3)$是一个32位整数，前16位表示影响该顶点的骨骼在$B$中的索引，后16位表示该顶点的影响权重。骨骼集合中每一个骨骼$b \in B$表示其相对它父骨骼$p_b \in B \bigcup \emptyset$的空间变换，若$p_b = \emptyset$，则骨骼$b$就是骨架的根骨骼。一般情况下，骨骼$b$可以用一个$4 \times 3$矩阵表示它的位置、旋转和缩放信息，但本文为了更好的说明PAQSlim算法，选择用一个四元数表示一个骨骼相对其父骨骼的旋转，即$b = (x, y, z, w)$。
- QEM改进（基于姿势概率的积分）  
- DQME实现（蒙特卡洛积分实现）  
- 概率分布函数讨论  
#### 4.2.2 可并发的网格简化算法改进  
